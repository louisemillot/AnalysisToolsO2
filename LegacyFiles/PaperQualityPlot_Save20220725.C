...
const Int_t InvMassRebinFactor_standard[numPart] = {15,3,3}; // I like 10 best for K0s at InvMass 0.4 to 0.6 with 400 bins, do variations around that
...
// Double_t pTbins[numPart][4] = {{0.2, 0.3},{0.6, 3.},{0.6, 3.}};
...
std::array<Double_t, 3> GetTotalAndBackgroundCount_BackgroundIntegration(TH1D* H1D_detectedV0s_DataLike_SmallpTinterval, Double_t SignalMean, Double_t SignalStandardDev, Double_t SideBandSizeMultiplierModifier, TFitResultPtr FitResult, TF1 *bkgparab) {
  ...
  cout << "Sigma_integral_fromRootFunction = " << Sigma_integral_fromRootFunction << ", sqrt = " << sqrt(1./((RightBackgroundLowerBound-LeftBackgroundUpperBound)/nbinInvMass)) << endl;
