...
void Draw_TH1_Histograms(TH1** histograms_collection, TString** pdfLegend_collection, Int_t collectionSize, TString* pdfName, TString* &texXtitle, TString* &texYtitle, Double_t* bins, Int_t nbins);
...
const Int_t SignalExtractionType = 0; //default is 1 for integration as opposed to 0 for bin counting; if using bin counting (0) then make sure H3D inv mass window is large enough to accomodate the 12sigma on both sides!
const Int_t InvMassRebinFactor_standard[numPart] = {10,5,5};
...
void PaperQualityPlot_FixAttempt() {
...
void CorrectedYield_withSystematics(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, Int_t ipart, TFile** file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  ...
  Get_Systematics_Fit_SideBandVariation(hSystematicUncertainty_Fit_SidebandVariation, hSystematicUncertainty_Fit_SidebandVariation_PreBarlow, ipart, H3D_detectedV0s_DataLike, H1I_SelectedEventCount, H3D_detectedV0s_TruePt, TrueV0PtSpectrum, pTbins, nbinpT, InvMassRebinFactor);
  Get_Systematics_Fit_SignalExtractionType(hSystematicUncertainty_Fit_SignalExtractionType, hSystematicUncertainty_Fit_SignalExtractionType_PreBarlow, ipart, H3D_detectedV0s_DataLike, H1I_SelectedEventCount, H3D_detectedV0s_TruePt, TrueV0PtSpectrum, pTbins, nbinpT, InvMassRebinFactor); //very large systematics as is
  // Get_Systematics_OneCut_McTrueAndMcDatalikeDifferentCuts(hSystematicUncertainty, hSystematicUncertainty_PreBarlow, ipart, file_O2Analysis_CutVariation_Datalike_array, file_O2Analysis_CutVariation_True_array, id_referenceAnalysis, nCutsIterations, pTbins, nbinpT, SignalExtractionType, InvMassRebinFactor);
...
/////////// Draw Efficiency /////////////
  TString* pdfName0 = new TString("");;
  *pdfName0 += "Efficiency";

  TH1* histograms_collection_0[1] = {hMcEfficiency_vsPt};
  TString* pdfLegend_collection_0[1] = {new TString("hMcEfficiency_vsPt")};
  Draw_TH1_Histograms(histograms_collection_0, pdfLegend_collection_0, 1, pdfName0, texXtitle, texYtitle, pTbins, nbinpT);


  /////////// Draw Corrected yield comparison O2 vs HEPdata /////////////
  TString* pdfName1 = new TString("");;
  *pdfName1 += "CorrectewdYields_O2_vs_HEPdata_vs_MCgenYield";
  // O2 MC true yield
  ...
  TH1* histograms_collection_1[3] = {hCorrectedYield_vsPt, hPtDiff_RawSpectrum_HEP[ipart], TrueV0PtSpectrum_rebinned};
  TString* pdfLegend_collection_1[3] = {new TString("hCorrectedYield_vsPt"), new TString("hPtDiff_RawSpectrum_HEP"),new TString("TrueV0PtSpectrum_rebinned")};
  Draw_TH1_Histograms(histograms_collection_1, pdfLegend_collection_1, 3, pdfName1, texXtitle, texYtitle, pTbins, nbinpT);
  ...
  Ratio_HEP_to_CorrectewdYield->Divide(hPtDiff_RawSpectrum_HEP[ipart],hCorrectedYield_vsPt);
  // Ratio_HEP_to_CorrectewdYield->Divide(hPtDiff_RawSpectrum_HEP[ipart],TrueV0PtSpectrum_rebinned);

  TH1* histograms_collection_2[2] = {Ratio_CorrectewdYield_to_MCgenYield, Ratio_HEP_to_CorrectewdYield};
  TString* pdfLegend_collection_2[2] = {new TString("Ratio_CorrectewdYield_to_MCgenYield"),new TString("Ratio_HEP_to_CorrectewdYield")};
  ...
  Draw_TH1_Histograms(histograms_collection_2, pdfLegend_collection_2, 2, pdfName2, texXtitle, texYtitle, pTbins, nbinpT);
...
void Test_LambdaTrueMCYieldvsHEP(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, Int_t ipart, TFile* file_O2Analysis, Int_t nCutsIterations, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  ...
  *pdfName += "CorrectewdYields_O2_vs_HEPdata";
  ...
  Draw_TH1_Histograms(histograms_collection, pdfLegend_collection, 2, pdfName, texXtitle, texYtitle, pTbins, nbinpT);
