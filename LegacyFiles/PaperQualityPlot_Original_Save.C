...
void Get_Systematics_OneCut(TH1D** hSystematicUncertainty, TH1D** hSystematicUncertainty_PreBarlow, Int_t ipart, TFile **file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, Double_t* pTbins, Int_t nbinpT);
void Get_RawYield_vsPt(TH1D* &hRawYield_vsPt, TH3D* H3D_detectedV0s_DataLike, TH1I* H1I_SelectedEventCount, Int_t ipart, Double_t* pTbins, Int_t nbinpT, Int_t ibinXaxisCut_low,  Int_t ibinXaxisCut_high, Int_t warning_cutArry_ID);
...
void Get_McEfficiency_vsPt(TH1D* &hMcEfficiency_vsPt, TH3D* H3D_detectedV0s_TruePt, TH1D* TrueV0PtSpectrum, Int_t ipart, Double_t* pTbins, Int_t nbinpT, Int_t ibinXaxisCut_low, Int_t ibinXaxisCut_high) {
  ...
  hMcEfficiency_vsPt->Divide(hMcSignalCount_vsPt,TrueV0PtSpectrum_AnalysisBins, 1., 1., "b");
...
void Get_RawYield_vsPt(TH1D* &hRawYield_vsPt, TH3D* H3D_detectedV0s_DataLike, TH1I* H1I_SelectedEventCount, Int_t ipart, Double_t* pTbins, Int_t nbinpT, Int_t ibinXaxisCut_low,  Int_t ibinXaxisCut_high, Int_t warning_cutArry_ID) {
  ...
  TH1D *H1D_detectedV0s_DataLike_SmallpTinterval[nbinpT];
  TH1D *hPtDiff_RawSpectrum_vsPt = new TH1D("hPtDiff_RawSpectrum_vsPt","hPtDiff_RawSpectrum_vsPt",nbinpT,pTbins);
  ...
  for(int ibinPt = 1; ibinPt <= nbinpT; ibinPt++){
    ...
    //NEW signal extraction, better:
    float SignalMean = parGaussParab[iPt][1];
    float SignalStandardDev = parGaussParab[iPt][2];
    int MeanMinus12Sigma_bin = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean - 12*SignalStandardDev);
    int MeanMinus6Sigma_bin = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean - 6*SignalStandardDev);
    int MeanPlus6Sigma_bin = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean + 6*SignalStandardDev);
    int MeanPlus12Sigma_bin = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean + 12*SignalStandardDev);

    int TotalCountInSignalRegion = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->Integral(MeanMinus6Sigma_bin, MeanPlus6Sigma_bin);

    // samples background in the two sidebands chosen: here mean-12sigma to mean-6sigma and mean+6sigma to mean+12sigma
    ...
    int BackgroundCountInSidebands = H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->Integral(MeanMinus12Sigma_bin, MeanMinus6Sigma_bin) + H1D_detectedV0s_DataLike_SmallpTinterval[iPt]->Integral(MeanPlus6Sigma_bin, MeanPlus12Sigma_bin); // symmetric sampling windows: mean-12sigma to mean-6sigma and mean+6sigma to mean+12Sigma
    ...
    float BackgroundExtrapolationInSignalRegion = (float)BackgroundCountInSidebands; 
    float SignalCount = TotalCountInSignalRegion - BackgroundExtrapolationInSignalRegion;
    ...
    float dpT = hPtDiff_RawSpectrum_vsPt->GetXaxis()->GetBinWidth(ibinPt);
    float drapidity = 1.5; //1.5
    double dN_dpT = SignalCount *1./dpT*1./drapidity;
    hPtDiff_RawSpectrum_vsPt->SetBinContent(ibinPt,dN_dpT);
    hPtDiff_RawSpectrum_vsPt->SetBinError(ibinPt,sqrt(SignalCount*1./dpT*1./drapidity)); //Issue with errors; this error of the O2 data spectrum is false, not right formula with fit
    
    // cout << "(ibinPt, SignalCount, trueV0count_ibinpT) = (" << ibinPt << ", " << SignalCount << ", " << trueV0count_ibinpT << ")" << endl;
    // cout << "efficiency = " << SignalCount*1./trueV0count_ibinpT << endl;
  }
  ////////////////////////////////// Fit End //////////////////////////////////
  // hSignalBackgroundRatio_vsPt = (TH1D*)hSignalCountFit_vsPt->Clone("hSignalBackgroundRatio_vsPt");
  // hSignalBackgroundRatio_vsPt->Divide(hSignalCountFit_vsPt,hBackgroundCountFit_vsPt);
  hRawYield_vsPt = (TH1D*)hPtDiff_RawSpectrum_vsPt->Clone("hRawYield_vsPt");
  hRawYield_vsPt->Scale(1./SelectedEventCount);

  delete hPtDiff_RawSpectrum_vsPt;
}
...
void Get_Systematics_OneCut(TH1D* hSystematicUncertainty, TH1D* hSystematicUncertainty_PreBarlow, Int_t ipart, TFile **file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, Double_t* pTbins, Int_t nbinpT) {
  ...
  for(int cutsArrayIterator = 0; cutsArrayIterator < nCutsIterations; cutsArrayIterator++){
    ...
    Get_RawYield_vsPt(hRawYield_vsPt[cutsArrayIterator], H3D_DetectedV0s_MCdatalike_cutVariation[cutsArrayIterator], H1I_SelectedEventCount[cutsArrayIterator], ipart, pTbins, nbinpT, 0, -1, cutsArrayIterator); ///real line that need to take the place of the one above once it's been written
    ...
    // }
    hCorrectedYield_vsPt[cutsArrayIterator] = (TH1D*)hRawYield_vsPt[cutsArrayIterator]->Clone("hCorrectedYield_vsPt");
  ...
  for(int ibinPt = 1; ibinPt <= nbinpT; ibinPt++){
    ...
    hSigmaBarlow[PtArrayIterator] = sqrt(abs(StatUncertainty_MaxDeviationCase*StatUncertainty_MaxDeviationCase - StatUncertainty_REF*StatUncertainty_REF));
    
    hSystematicUncertainty_PreBarlow->SetBinContent(ibinPt,SystUncertainty*1./hCorrectedYield_vsPt_REF->GetBinContent(ibinPt)); //get it as a percentage of ref yield

    if (SystUncertainty > 2*hSigmaBarlow[PtArrayIterator]) { //Could ask for 1Sigma, 4Sigma or whatever depending on how conservative we want to be; one suggested in PWGLF note is 2Sigma
      hSystematicUncertainty->SetBinContent(ibinPt,SystUncertainty*1./hCorrectedYield_vsPt_REF->GetBinContent(ibinPt)); //get it as a percentage of ref yield
    }
    else {
    ...
    cout << "(SystUncertainty, SigmaBarlow) = (" << SystUncertainty << "," << hSigmaBarlow[PtArrayIterator] << ") and check is " << (SystUncertainty > 2*hSigmaBarlow[PtArrayIterator]) << endl;
    cout << "hCorrectedYield_vsPt_REF->GetBinError(ibinPt) = " << hCorrectedYield_vsPt_REF->GetBinError(ibinPt) << endl;
  }
...
void Systematics_CutVariations_Graphs(TH1D* &hstat, TH1D* &hsyst, TH1D*&hsystCorr, Int_t ipart, TFile** file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  ...
  TH1D* hSystematicUncertainty = new TH1D("hSystematicUncertainty", "hSystematicUncertainty", nbinpT, pTbins[0], pTbins[nbinpT]);
  TH1D* hSystematicUncertainty_PreBarlow = new TH1D("hSystematicUncertainty_PreBarlow", "hSystematicUncertainty_PreBarlow", nbinpT, pTbins[0], pTbins[nbinpT]);


  Get_Systematics_OneCut(hSystematicUncertainty, hSystematicUncertainty_PreBarlow, ipart, file_O2Analysis_CutVariation_array, id_referenceAnalysis, nCutsIterations, pTbins, nbinpT);
  ...
  TH1 *hFrame = canvasSyst->DrawFrame(pTbins[0],0,pTbins[nbinpT],1.2*hSystematicUncertainty_PreBarlow->GetMaximum());
  hFrame->SetXTitle("#it{p}_{T} (GeV/#it{c})");
  hFrame->SetYTitle("uncertainty (%)");

  hSystematicUncertainty->Draw("same");
  hSystematicUncertainty_PreBarlow->Draw("same");

  Int_t icolor1=0;
  Int_t icolor2=1;

  hSystematicUncertainty->SetMarkerStyle(markers[0]);
  hSystematicUncertainty->SetMarkerColor(colors [icolor1]);
  hSystematicUncertainty->SetLineColor  (colors [icolor1]);

  hSystematicUncertainty_PreBarlow->SetMarkerStyle(markers[0]);
  hSystematicUncertainty_PreBarlow->SetMarkerColor(colors [icolor2]);
  hSystematicUncertainty_PreBarlow->SetLineColor  (colors [icolor2]);

  canvasSyst->SaveAs("Systematics_CutVariations_vsPt_"+NamePart[ipart]+".pdf");

    // also fo systematics we want a percentage of the yield, so gotta do : errorYield/yield
  ...
  hstat = (TH1D*)hSystematicUncertainty->Clone("hstat");
  hstat->Reset("M");
  // hstat->Divide(hPtDiff_RawSpectrum_O2,hPtDiff_RawSpectrum_HEP[ipart]);
...
// void Get_Systematics_Fit(TH1D* &hSystematicUncertainty, Int_t ipart, TFile* file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT, bool isMC) {
