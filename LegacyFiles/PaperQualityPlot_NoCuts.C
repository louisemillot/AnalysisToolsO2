...
void Get_McEfficiency_vsPt(TH1D* &hMcEfficiency_vsPt, TH3D* H3D_detectedV0s_TruePt, TH1D* TrueV0PtSpectrum, Int_t ipart, Double_t* pTbins, Int_t nbinpT, Int_t ibinXaxisCut_low, Int_t ibinXaxisCut_high, Double_t SideBandSizeMultiplierModifier, Float_t SignalMean, Float_t SignalStandardDev) {
  ...
  for(int ibinPt = 1; ibinPt <= nbinpT; ibinPt++){
    ...
    int ibinPt_signalRange_low = H1D_detectedV0s_TruePt_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean - (4 + SideBandSizeMultiplierModifier)*SignalStandardDev);
    int ibinPt_signalRange_up = H1D_detectedV0s_TruePt_SmallpTinterval[iPt]->GetXaxis()->FindBin(SignalMean + (4 + SideBandSizeMultiplierModifier)*SignalStandardDev);
    cout << "(ibinPt_signalRange_low, ibinPt_signalRange_up) = (" << ibinPt_signalRange_low << ", " << ibinPt_signalRange_low << ")" << endl;

    float SignalCount_SignalRange = H1D_detectedV0s_TruePt_SmallpTinterval[iPt]->Integral(ibinPt_signalRange_low, ibinPt_signalRange_up); // avoids overflow bins 0 and -1 ; without fit
...
void Systematics_CutVariations_Graphs(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, Int_t ipart, TFile** file_O2Analysis_CutVariation_array, Int_t id_referenceAnalysis, Int_t nCutsIterations, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  ...
  Double_t SignalMean, SignalStandardDev;
  Get_RawYield_vsPt(hRawYield_vsPt, H3D_detectedV0s_DataLike, H1I_SelectedEventCount, &SignalMean, &SignalStandardDev, ipart, pTbins, nbinpT, 0, -1, 0, SideBandSizeMultiplierModifier_array[0], SignalExtractionType_default, InvMassRebinFactor); ///real line that need to take the place of the one above once it's been written
