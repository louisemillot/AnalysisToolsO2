...
void Draw_FinderEfficiencies(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis_Run1Cuts, TFile *file_O2Analysis_SVCuts, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT);
void Draw_XiChargedToNeutralRatio(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT);

//PhD_Amendments
void Draw_Mass_Histograms_multipleRuns(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart);
void Draw_TH1_Histograms_Runs(TH1D** histograms_collection, Int_t collectionSize, TString* pdfName, TString* &texXtitle, TString* &texYtitle);
void Draw_EfficiencyWithSigmoidFit(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT);
void Get_McEfficiency_vsPt_alwaysFitSmooth(TH1D* &hMcEfficiency_vsPt, TH3D* H3D_detectedV0s_TruePt, TH1D* TrueV0PtSpectrum, Int_t ipart, Double_t* pTbins, Int_t nbinpT, Int_t ibinXaxisCut_low, Int_t ibinXaxisCut_high, Double_t SideBandSizeMultiplierModifier, Float_t SignalMean, Float_t SignalStandardDev, Int_t InvMassRebinFactor);
void Draw_PtResponseMatrix(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT);
void Get_PtResponseMatrix(TH2D * H2D_PtResponseMatrix, TFile *file_O2Analysis, Double_t* pTbins, Int_t nbinpT);
...
Double_t fgenerallogistic(Double_t *x, Double_t *par);
...
const Float_t N_SigmaBarlow = 1.9; //often 2 in ALICE LF analyses
...
// const Int_t nRuns = 8;
// const Int_t runList[nRuns] = {505548, 505582, 505600, 505629, 505637, 505645, 505658, 505669};
...
// const Int_t colors_Runs[] = {kCyan, kAzure+7, kBlue, kMagenta+3, kPink+5, kRed, kOrange+1, kOrange-2}; // for syst bands
// const Int_t markers_Runs[] = {kFullSquare, kFullSquare, kFullSquare, kFullCross,kFullCross,kFullCross,kFullCross,kFullCross,kFullCross,kFullCross};


const Int_t nRuns = 2; // 8;
const Int_t runList[nRuns] = {505548, 505582};
...
TFile* file_O2Analysis_RunComparison_array[nRuns] = {file_O2Analysis1, file_O2Analysis2}; //, file_O2Analysis3, file_O2Analysis4, file_O2Analysis5, file_O2Analysis6, file_O2Analysis7, file_O2Analysis8};
const Int_t colors_Runs[] = {kBlue, kRed}; // for syst bands
const Int_t markers_Runs[] = {kFullSquare, kFullCircle};
...
//   Double_t TotalCountInSignalRegion_SystError = sqrt(TotalCountInSignalRegion*TotalCountInSignalRegion); // AIMERIC du futur: pretty sure this should be sqrt(TotalCountInSignalRegion) simply
  Double_t TotalCountInSignalRegion_SystError = sqrt(TotalCountInSignalRegion); // AIMERIC du futur: pretty sure this should be sqrt(TotalCountInSignalRegion) simply
...
void Get_FeedDown(TH1D * H1D_FeedDownCorrection, Int_t ipart_option, TFile *file_O2Analysis, Double_t* pTbins, Int_t nbinpT) {
  cout << "Get_FeedDown DEBUG 1" << endl;
  ...
  cout << "Get_FeedDown DEBUG 2" << endl;
  ...
  TH1I* H1I_SelectedEventCount = (TH1I*)file_O2Analysis->Get("lambdakzero-analysis-mc/hSelectedEventCounter");
  cout << "Get_FeedDown DEBUG 3" << endl;

  cout << "______Rebinning:" << endl;
  for(Int_t i = 1; i <= nbinpT; i++){
  cout << "Get_FeedDown DEBUG 3.1 i=" << i << endl;
    Int_t arrayID_i = i - 1;
    ...
    for(Int_t j = 1; j <= nbinpT_j; j++){
      cout << "Get_FeedDown DEBUG 3.2 i=" << i << ",j=" << j << endl;
      Int_t arrayID_j = j - 1;
      ...
      cout << "Get_FeedDown DEBUG 3.3 i=" << i << ",j=" << j << ", integral =" << H2D_Feeddown_numerator->Integral(LowerBound_bin_i,UpperBound_bin_i,LowerBound_bin_j,UpperBound_bin_j) << endl;
      H1D_Feeddown_denominator_rebinned->SetBinContent(j, H1D_Feeddown_denominator->Integral(LowerBound_bin_j,UpperBound_bin_j));
      cout << "Get_FeedDown DEBUG 3.4 i=" << i << ",j=" << j << endl;
    }
  }
  cout << "Get_FeedDown DEBUG 4" << endl;
  ...
  cout << "Get_FeedDown DEBUG 5" << endl;
  ...
  cout << "Get_FeedDown DEBUG 6" << endl;
  ...
  cout << "Get_FeedDown DEBUG 7" << endl;
  ...
  TMatrixDSym covMatrix = fFitResult->GetCovarianceMatrix();
  cout << "Get_FeedDown DEBUG 8" << endl;
  ...
  cout << "Get_FeedDown DEBUG 9" << endl;
...
void Test_LambdaTrueMCYieldvsHEP(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, Int_t ipart, TFile* file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  ...
  for (Int_t ibinPt = 0; ibinPt < nbinpT; ibinPt++)
  {
    Double_t dpT = TrueV0PtSpectrum_rebinned->GetXaxis()->GetBinWidth(ibinPt);
    Double_t drapidity = 1.5; //1.5
    Double_t d2N_dpTdy = TrueV0PtSpectrum_rebinned->GetBinContent(ibinPt) *1./dpT*1./drapidity; //note from Aimeric du futur: should be sqrt(getbincontent)
    TrueV0PtSpectrum_rebinned->SetBinContent(ibinPt, d2N_dpTdy);
  }
...
void Draw_FinderEfficiencies(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis_Run1Cuts, TFile *file_O2Analysis_SVCuts, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT){
  ...
  Double_t SignalMean_Run1Cuts, SignalStandardDev_Run1Cuts;
  ...
  Double_t SignalMean_SVCuts, SignalStandardDev_SVCuts;
...
void Draw_XiChargedToNeutralRatio(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT){
  ...
  // Int_t ratioOption = 1; //to draw line at ratio=1
  // texXtitle = texPtX;
  // texYtitle = texRatio;
  ...
  Int_t nbinpTxi = ratio_XiChargedToNeutral->GetNbinsX();
  for(Int_t ibinPt = 1; ibinPt <= nbinpTxi; ibinPt++){
    Float_t pT = ratio_XiChargedToNeutral->GetXaxis()->GetBinCenter(ibinPt);
...
Double_t fpolysixthorder(Double_t *x, Double_t *par) {
...
Double_t fsigmoid(Double_t *x, Double_t *par) {
  return par[0] / (1 + exp(-par[1] * (x[0]-par[2]))); //    y = L / (1 + exp(-k*(x-x0))) + b
}
Double_t fgompertz(Double_t *x, Double_t *par) {
...
void Draw_EfficiencyWithSigmoidFit(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Int_t ipart, Double_t* pTbins, Int_t nbinpT){
  ...
  // Draw_TH1_Histograms_Efficiency(histograms_collection_0, pdfLegend_collection_0, 2, pdfName0, texXtitle, texYtitle, pTbins, nbinpT, ratioOption0);
  // void Draw_TH1_Histograms_Efficiency(TH1** histograms_collection, TString** pdfLegend_collection, Int_t collectionSize, TString* pdfName, TString* &texXtitle, TString* &texYtitle, Double_t* bins, Int_t nbins, Int_t ratioOption) {
  Float_t MaximumY = hMcEfficiency_vsPt_SVCuts->GetMaximum();
...
void Draw_PtResponseMatrix(TH1D* &hstat, TH1D* &hsyst, TH1D* &hsystCorr, TFile *file_O2Analysis, TString* SaveAs_Title, TString* &texXtitle, TString* &texYtitle, Double_t* pTbins, Int_t nbinpT) {
  TH2D* H2D_PtDataVsMC = (TH2D*)file_O2Analysis->Get("thesis-plots-cut-distribs-signal-bkg/hPtDataVsMC_"+NamePart[ipart]);
  ...
  for(Int_t i = 1; i <= nbinpT; i++){
    Int_t arrayID_i = i - 1;
    Int_t LowerBound_bin_i = H2D_PtDataVsMC->GetXaxis()->FindBin(pTbins[arrayID_i]);
    Int_t UpperBound_bin_i = H2D_PtDataVsMC->GetXaxis()->FindBin(pTbins[arrayID_i+1]) - 1;
    cout << "LowerBound bin_i " << i << " = " << LowerBound_bin_i << ", UpperBound bin_i " << i << " = " << UpperBound_bin_i << endl;
    for(Int_t j = 1; j <= nbinpT; j++){
      Int_t arrayID_j = j - 1;
      Int_t LowerBound_bin_j = H2D_PtDataVsMC->GetYaxis()->FindBin(pTbins[arrayID_j]);
      Int_t UpperBound_bin_j = H2D_PtDataVsMC->GetYaxis()->FindBin(pTbins[arrayID_j+1]) - 1;
      cout << "LowerBound bin_j " << j << " = " << LowerBound_bin_j << ", UpperBound bin_j " << j << " = " << UpperBound_bin_j << endl;
...
void Get_PtResponseMatrix(TH2D * H2D_PtResponseMatrix, TFile *file_O2Analysis, Double_t* pTbins, Int_t nbinpT){
  ...
  H2D_PtResponseMatrix = (TH2D*)H2D_PtDataVsMC_rebinned_density->Clone("H2D_PtResponseMatrix");
}
